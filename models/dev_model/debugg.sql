with
    ...
    t2 as (select * from {{ ref("dev_stg_cdm1") }}),
    t3 as (select * from t1 Left join t2 on t1.student_barcode = t2.assessment_barcode 
    order by t1.student_barcode)

select * from t3
where q4_1 is not null and q4_2 is not null and student_barcode is not null and batch_academic_year = 2025
