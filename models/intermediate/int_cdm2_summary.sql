...
t3 as (select '1b. stg_cdm2' as table_name, column_name, (select count (*) from t0) as total_records, 
...
t3 as (select '2b. int_cdm2_latest' as table_name, column_name, (select count (*) from t0) as total_records, 
(case when null_count is not null then null_count else 0 end) as null_count, distinct_count from t1 full outer join t2 using(column_name))
...
t0 as (select * from {{ ref("int_pivot_cdm2_latest") }}),
...
t3 as (select '3b. int_pivot_cdm2_latest' as table_name, column_name, (select count (*) from t0) as total_records, 
(case when null_count is not null then null_count else 0 end) as null_count, distinct_count from t1 full outer join t2 using(column_name))
...
t0 as (select * from {{ ref("int_student_cdm2_pivot_global") }}),
...
t3 as (select '4b. int_student_cdm2_pivot_global' as table_name, column_name, (select count (*) from t0) as total_records, 
(case when null_count is not null then null_count else 0 end) as null_count, distinct_count from t1 full outer join t2 using(column_name))
