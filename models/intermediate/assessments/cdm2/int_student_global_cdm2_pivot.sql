with
    ...
    t2 as (select * from {{ ref("int_pivot_cdm2_latest") }}),
    t3 as (select t1.*, t2.barcode as assessment_barcode, 
    t2.* except(barcode) 
    from t1 full outer join t2 on t1.contact_barcode = t2.barcode 
