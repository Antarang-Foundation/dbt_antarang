with
    t0 as (
        SELECT 
                ...
                created_on, grade, academic_year
    ...
    t1 as (
        ...
        from t0
            PIVOT (
            max(cs_no) as cs_no, max(batch_id) as batch_id, max(created_on) as created_on, max(grade) as grade, max(academic_year) as academic_year,
