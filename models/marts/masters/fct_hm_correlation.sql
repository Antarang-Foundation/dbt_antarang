WITH hm_session AS (
    SELECT 
        ...
        COUNT(CASE WHEN LOWER(TRIM(session_type)) = 'student' THEN 1 END) AS expected_student_sessions,
        COUNT(CASE WHEN LOWER(TRIM(session_type)) = 'parent' THEN 1 END) AS expected_parent_sessions,

        SUM(CASE WHEN LOWER(TRIM(session_type)) = 'student' 
        ...
        SUM(CASE WHEN LOWER(TRIM(session_type)) = 'parent' 
                  AND hm_session_date IS NOT NULL 
                  AND total_student_present IS NOT NULL 
    ...
    GROUP BY hm_school_id, facilitator_name,session_academic_year, batch_language, fac_start_date, fac_end_date, school_name,
        school_taluka, school_district, school_state, school_area, school_partner, batch_expected_sessions
),
...
hm_assessment as (SELECT 
school_name as ass_school_name,

pre_start, pre_end, pre_date, Q4_pre__years_in_this_school, Q5_pre_organisation_last_year, Q5_pre_option_1, Q5_pre_option_2,
Q5_pre_option_3, Q6_pre_heard_of_antarang, Q7_pre_how_did_you_hear, Q7a_newspapers, Q7b_pre_social_media__facebook__linked, 
